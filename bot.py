...
# ──────────────────────────────────────────────────────────────────────────
# Токен бота и ключ Supabase читаются из переменных окружения (Railway →
# Variables). Если переменная не задана — используется старое хардкоженное
# значение как запасной вариант, чтобы бот не упал сразу при первом переходе
# на env-переменные, но настоятельно рекомендуется задать переменные в
# Railway и убрать значения отсюда.
#
# Важно про SUPABASE_KEY: используй именно service_role ключ (Supabase →
# Settings → API → service_role), а не anon/publishable — иначе запросы на
# запись (INSERT/UPDATE) будут падать с ошибкой row-level security policy,
# если в таблицах включена RLS без явных policy на запись.
# service_role даёт полный доступ к базе в обход RLS — НИКОГДА не используй
# его на фронтенде/в браузере, только на сервере (как здесь, в боте на Railway).
# ──────────────────────────────────────────────────────────────────────────
API_TOKEN = os.environ.get("BOT_TOKEN", "8942639067:AAGgub5pba2LK2WLjJ6n1KpJGfmGcZpVMCQ")
ADMIN_ID = int(os.environ.get("ADMIN_ID", "8915415360"))

SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://umjmcdvjdqlsjmphvsfc.supabase.co")
SUPABASE_KEY = os.environ.get("SUPABASE_KEY", "sb_publishable_4hpLHc_Wxl323Y5nGODAdQ_3vle6d2t")
