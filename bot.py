...
async def my_profile(callback: CallbackQuery):
    ...
    text = f"👤 *Твой профиль*\n\n"
    text += f"📅 Дней подписки: *{days}*\n"
    text += f"📆 Подписка до: *{end_date}*\n"
    text += f"🔗 Пригласил: *{ref_by}*\n"
    text += f"💰 Всего оплат в боте: *{total_payments}*\n\n"
    ...
    kb = [back_button()]
    if not license_key and not is_subscribed(user_id):
        kb.append([InlineKeyboardButton(text="🔑 Ввести ключ", callback_data="enter_key")])
...
async def process_enter_key(message: types.Message, state: FSMContext):
    ...
    activate_subscription(message.from_user.id, days_left)
...
async def send_cheat(callback: CallbackQuery):
    ...
    await callback.message.answer(
        f"📁 *Чит для {game.upper()}*\n\n"
        f"🔑 *Твой ключ:*\n`{license_key}`\n"
        f"📅 Действителен до: {expires}\n\n"
        f"📱 *Скачай .apk:*\n[Скачать {game.upper()}]({apk_link})\n\n"
...
async def process_admin_create_key(message: types.Message, state: FSMContext):
    try:
        ...
        existing = supabase.table("keys").select("*").eq("user_id", user_id).execute()
        if existing.data:
        ...
        await message.answer("❌ Ошибка! Формат: `ID 30`", parse_mode="Markdown")
    await state.clear()
